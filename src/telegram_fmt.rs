...
use crate::channel::telegram::escape_html;
    ...
    for event in parser {
        match event {
            ...
            Event::End(tag_end) => match tag_end {
                ...
                TagEnd::CodeBlock => {
                    // check if we used <pre><code> or just <pre>
                    // look back to see if there's an unclosed <code>
                ...
                TagEnd::Item => {
                    // trim trailing paragraph newlines inside list items
                    while out.ends_with('\n') {
                        out.pop();
                    }
                    out.push('\n');
