...
pub const ALLOWED_MODELS: &[&str] = &["deepseek-v4-flash", "deepseek-v4-pro"];
...
impl DeepSeekProvider {
    ...
    pub fn from_env(client: Client) -> Result<Self, Error> {
        let api_key = std::env::var("DEEPSEEK_API_KEY")
            .map_err(|_| Error::MissingApiKey("DEEPSEEK_API_KEY"))?;
    ...
    pub fn context_window(&self) -> u32 {
        match self.model.as_str() {
            "deepseek-v4-pro" | "deepseek-v4-flash" => 1_048_576,
            _ => 128_000,
        }
    }
}
...
fn thinking_request(effort: ReasoningEffort) -> ThinkingRequest {
    ThinkingRequest {
        thinking_type: match effort {
...
impl Provider for DeepSeekProvider {
    ...
    async fn complete(
        ...
        let request = ApiRequest {
            ...
            thinking: Some(thinking_request(self.reasoning_effort)),
        ...
        let choice = api_response
            ...
            .into_iter()
            .next()
            .ok_or_else(|| Error::Provider("empty response from deepseek".into()))?;
...
mod tests {
    ...
    fn test_allowed_models() {
        ...
        assert!(ALLOWED_MODELS.contains(&"deepseek-v4-pro"));
        assert!(ALLOWED_MODELS.contains(&"deepseek-v4-flash"));
    }

    #[test]
    fn test_request_enables_thinking_for_high_effort() {
        let req = ApiRequest {
            model: "deepseek-v4-pro",
            messages: vec![],
            tools: vec![],
            max_tokens: 1024,
            thinking: Some(thinking_request(ReasoningEffort::High)),
    ...
    #[test]
    fn test_request_maps_xhigh_to_max() {
        let req = ApiRequest {
            model: "deepseek-v4-pro",
            messages: vec![],
            tools: vec![],
            max_tokens: 1024,
            thinking: Some(thinking_request(ReasoningEffort::XHigh)),
            reasoning_effort: reasoning_effort(ReasoningEffort::XHigh),
        };
        let body = serde_json::to_value(&req).unwrap();
        assert_eq!(body["thinking"]["type"], "enabled");
    ...
    #[test]
    fn test_request_disables_thinking_for_none() {
        let req = ApiRequest {
            model: "deepseek-v4-flash",
            messages: vec![],
            tools: vec![],
            max_tokens: 1024,
            thinking: Some(thinking_request(ReasoningEffort::None)),
            reasoning_effort: reasoning_effort(ReasoningEffort::None),
        };
        let body = serde_json::to_value(&req).unwrap();
    ...
    #[test]
    fn test_convert_assistant_preserves_reasoning_content() {
        let messages = vec![Message::assistant_with_content(vec![
            MessageContent::thinking("private chain", ""),
            MessageContent::text("calling tool"),
        ...
        assert_eq!(
            result[1].reasoning_content.as_deref(),
            Some("private chain")
        );
        assert_eq!(
        ...
        assert_eq!(
            result[1].tool_calls.as_ref().unwrap()[0].function.name,
            "remember"
        );
    }
